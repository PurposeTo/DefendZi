...
namespace Desdiene.Tools
{
    public class InternetConnectionChecker
    {
        ...
        /// <returns></returns>
        public IEnumerator PingGoogle(Action<bool> isInternetAvailable, int timeOut = 5)
        {
            ...
            using (UnityWebRequest request = new UnityWebRequest("https://google.com", "GET"))
            {
                ...
                if (request.error != null || request.result == UnityWebRequest.Result.ProtocolError)
                {
        ...
        public IEnumerator PingGoogle(Action<bool> isInternetAvailable)
        {
            // при timeOut = 0 тайм аут отсутствует, операция прервана не будет.
            yield return PingGoogle(isInternetAvailable, 0);
        }
    }
}
